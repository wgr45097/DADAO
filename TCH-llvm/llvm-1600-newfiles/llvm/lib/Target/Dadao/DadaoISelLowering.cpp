...
DadaoTargetLowering::DadaoTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::dadaoptr, &Dadao::GPRBRegClass);
  ...
  setOperationAction(ISD::BR_CC, MVT::dadaoptr, Custom);
  setOperationAction(ISD::BR_JT, MVT::Other, Expand);
  setOperationAction(ISD::BRCOND, MVT::Other, Expand);
  setOperationAction(ISD::SETCC, MVT::i64, Custom);
  setOperationAction(ISD::SETCC, MVT::dadaoptr, Custom);
  setOperationAction(ISD::SELECT, MVT::i64, Expand);
  setOperationAction(ISD::SELECT_CC, MVT::i64, Custom);
  setOperationAction(ISD::SELECT_CC, MVT::dadaoptr, Custom);
...
SDValue DadaoTargetLowering::LowerCCCArguments(
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    // TODO: set isRegLoc for pointers with dadaoptr
    if (VA.isRegLoc()) {
...
SDValue DadaoTargetLowering::LowerCallResult(
  ...
  for (unsigned I = 0; I != RVLocs.size(); ++I) {
    ...
    if (RVLocs[I].getValVT() != MVT::i64) {
      assert(RVLocs[I].getValVT() == MVT::dadaoptr && "Unsupported retval type conversion");
      Chain = SDValue(DAG.getMachineNode(Dadao::RD2RB_ORRI, DL, MVT::dadaoptr, MVT::Other, MVT::Glue,
